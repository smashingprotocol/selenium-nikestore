...
public class ShoppingCartAddtoCartfoLowerPriceDisplay {
	...
	@Test
	public void ShoppingCart_AddtoCart_foLower_Price_Display() throws Exception{
		...
		try{
			...
			for(int i = 1; i <= rowCtr; i++){
				...
				Assert.assertTrue("[CART] Expected Unit Price of " + unitPrice + " is display.",testStatus);
				...
				Assert.assertTrue("[CART] Expected Final Price of " + FinalPrice + " is display.",testStatus);
				
				
				
			} //end for
	
			StatusLog.printlnPassedResult(Config.driver,"[PCM] Shopping Cart AddtoCart for Lower Price Display");
		
		} catch (Exception e){
			StatusLog.printlnFailedResult(Config.driver,"[PCM] Shopping Cart AddtoCart for Lower Price Display");
