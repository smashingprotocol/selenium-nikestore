...
public class VerifyOrderAddtoCartfoLowerPriceSubTotal {
	...
	public void Verify_Order_AddtoCart_fo_Lower_Price_SubTotal() throws Exception{
		...
		try{
			...
			StatusLog.printlnPassedResult(Config.driver,"[PCM] Sub Total of Guest is equals to the carts Sub Total");
			...
			StatusLog.printlnPassedResult(Config.driver,"[PCM] Verify Order AddtoCart for Lower Price SubTotal in New Customer");
		
		} catch (Exception e){
			StatusLog.printlnFailedResult(Config.driver,"[PCM] Verify Order AddtoCart for Lower Price SubTotal");
